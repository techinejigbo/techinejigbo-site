...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validAdmin = await checkIfAdmin(user.uid);
      
      if (!validAdmin) {
        setError("Access Denied: You do not have administrator privileges.");
        // The AdminProvider will automatically log them out since they are not an admin
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <div className="w-16 h-16 bg-brand-orange-light/10 rounded-full flex items-center justify-center mb-4">
            <ShieldCheck size={32} className="text-brand-orange" />
          </div>
          <h1 className="text-2xl font-display font-bold text-slate-900 tracking-tight uppercase">Admin Portal</h1>
